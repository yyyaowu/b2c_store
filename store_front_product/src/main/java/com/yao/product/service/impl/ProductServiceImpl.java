...
import com.yao.product.mapper.PictureMapper;
...
public class ProductServiceImpl extends ServiceImpl<ProductMapper,Product> implements ProductService {
     ...
     * @return
     */
    @Cacheable(value = "list.product",key = "#productIdsParam.productIds",unless="#result == null")
    @Override
    ...
    public R sekillList() {
        List<Product> productList = null;
//        QueryWrapper<Seckill> seckillQueryWrapper = new QueryWrapper<>();
        List<Seckill> seckills = seckillMapper.selectList(new QueryWrapper<>());
        productList = seckills.stream().map(
                item -> {
                    Product product = new Product();
                    product = this.getById(item.getProductId());
                    product.setProductSellingPrice(item.getSeckillPrice());
                    return product;
                }
    ...
    @Override
    public R detailSeckill(Integer productID) {
        Product product = productMapper.selectById(productID);
        QueryWrapper<Seckill> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("product_id",productID);
        Seckill seckill = seckillMapper.selectOne(queryWrapper);
        product.setProductSellingPrice(seckill.getSeckillPrice());
        R ok = R.ok(product);
        log.info("ProductServiceImpl.detailSeckill业务结束，结果:{}",ok);
        return ok;
    }
